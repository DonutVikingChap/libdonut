...
void RenderPass::draw(ModelInstance&& model) {
	...
	const auto it = std::lower_bound(buffers->objectsSortedByShaderAndModel.begin(), buffers->objectsSortedByShaderAndModel.end(), model);
	if (it == buffers->objectsSortedByShaderAndModel.end()) {
		objectInstances = &buffers->objectsSortedByShaderAndModel.emplace_back(model.shader, model.model).objectInstances;
		...
		objectInstances = &buffers->objectsSortedByShaderAndModel.emplace(it, model.shader, model.model)->objectInstances;
		objectInstances->resize(objectCount);
	}
...
void RenderPass::draw(TextureInstance&& texture) {
	...
	draw(RectangleInstance{
		...
		.size = texture.texture->getSize() * texture.scale,
		.angle = texture.angle,
...
void RenderPass::draw(QuadInstance&& quad) {
	...
	const auto it = std::lower_bound(buffers->quadsSortedByShaderAndTexture.begin(), buffers->quadsSortedByShaderAndTexture.end(), quad);
	if (it == buffers->quadsSortedByShaderAndTexture.end()) {
		buffers->quadsSortedByShaderAndTexture.emplace_back(quad.shader, quad.texture).instances.push_back(instance);
	} else if (it->shader == quad.shader && it->texture == quad.texture) {
		it->instances.push_back(instance);
	} else {
		buffers->quadsSortedByShaderAndTexture.emplace(it, quad.shader, quad.texture)->instances.push_back(instance);
	}
}
...
void RenderPass::draw(TextInstance&& text) {
	...
	const auto it = std::lower_bound(buffers->glyphsSortedByFont.begin(), buffers->glyphsSortedByFont.end(), text);
	if (it == buffers->glyphsSortedByFont.end()) {
		instances = &buffers->glyphsSortedByFont.emplace_back(text.font).instances;
	} else if (it->font == text.font) {
		instances = &it->instances;
	} else {
		instances = &buffers->glyphsSortedByFont.emplace(it, text.font)->instances;
	}
