#ifndef DONUT_EVENTS_EVENT_HPP
#define DONUT_EVENTS_EVENT_HPP
...
struct MouseEventBase : InputEventBase {
	...
	ivec2 mousePosition;       ///< Current mouse position relative to the window.
	ivec2 relativeMouseMotion; ///< Position offset relative to the previous position.
};
...
struct ControllerAxisEventBase : ControllerEventBase {
	...
	i16 axisValue;       ///< Current axis value in the range [-32768, 32767].
};
...
struct WindowMovedEvent : WindowEventBase {
	ivec2 windowPosition; ///< Current window position.
};

/** Window was resized manually. */
struct WindowResizedEvent : WindowEventBase {
	ivec2 windowSize; ///< Current window size.
};
...
struct MouseWheelScrolledEvent : MouseEventBase {
	ivec2 scrollTicks; ///< Number of scrolls horizontally/vertically.
